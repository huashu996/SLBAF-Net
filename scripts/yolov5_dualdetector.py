...
sys.path.append(os.path.join(cwd, 'modules/yolov5-dual'))
...
class Yolov5Detector():
    def __init__(self, weights=''):
        ...
        # Load model
        weights = os.path.join(cwd, 'modules/yolov5-dual', weights)
    ...
    def imgdeal(self,img):
        ...
        if img.ndimension() == 3:
            img = img.unsqueeze(0)
        return img
    def run(self, img0,img2, conf_thres=0.25, iou_thres=0.45, classes=None):
        # Padded resize
        img1 = self.imgdeal(img0)
        ...
        if len(det):
            ...
            boxes = scale_coords(img1.shape[2:], det[:, :4], img0.shape).round().cpu().numpy() # xyxy
...
if __name__ == '__main__':
    ...
    detector = Yolov5Detector(weights='weights/dual1.pt')
    
    t1 = time.time()
    labels, scores, boxes = detector.run(
        img,img2,classes=[0, 1, 2, 3, 4]
